...
const model = genAI.getGenerativeModel({ 
    ...
    systemInstruction: `ඔබේ නම QUEEN SHENU. ඔබ මියුරු නිමේශ් විසින් සාදන ලද AI සහායකයෙකි.
...
export async function sendAndSaveMessage(sock, jid, text, senderName, msgId) {
    try {
        await sock.sendMessage(jid, { text });
        await db.collection('private_messages').insertOne({
            message_id: msgId, from_jid: 'bot', sender_name: senderName,
            text, timestamp: Date.now(), direction: 'outgoing'
        });
...
export async function handleAutoReply(sock, from, text, senderName, imageBuffer = null) {
    try {
        if (!(await quotaManager.check(from))) {
            return await sock.sendMessage(from, { text: "⚠️ අද දිනට AI සීමාව අවසන්! ⏰" });
        }

        let prompt = [text || "මේ පින්තූරයේ මොකක්ද තියෙන්නේ?"];
        if (imageBuffer) {
            prompt = [{ inlineData: { data: imageBuffer.toString("base64"), mimeType: "image/jpeg" } }, prompt[0]];
        }
...
export async function handleCallRejectReply(sock, callFrom) {
    try {
        const result = await model.generateContent("කවුරුහරි call එකක් දැම්මා answer කරන්න බැරිවුණා. කෙටි සිංහල reply එකක් දෙන්න.");
        const aiMessage = result.response.text().trim();
        const audio = await textToSpeech(aiMessage);
        if (audio) await sock.sendMessage(callFrom, { audio: audio, mimetype: 'audio/mpeg', ptt: true });
